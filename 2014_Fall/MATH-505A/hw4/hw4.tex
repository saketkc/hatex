...
\begin{homeworkProblem}[Exercise \# 2.3] % Custom section title
	...
	\begin{homeworkSection}{(1)} % Section within problem
		...
		\problemAnswer{ % Answer
			...
			so $F(x)$  approaches $G(x)$ $\forall x$ 
	...
	\begin{homeworkSection}{(2)} % Section within problem
		...
		\problemAnswer{ % Answer
			...
			Consider $\{Y \leq y\}$ $\implies \{g(X) \leq y\} $. Since $g^{-1}$ exists, such a set is equivalent to: $\{X \leq g^{-1}(y)\}$ which belongs to $\mathcal{F}$ as $g : R \rightarrow R$. Since the last equations hold true and hence $\{Y \leq y\} \in \mathcal{F}$ so $g(X)$ is a RV
	...
	\begin{homeworkSection}{(3)} % Section within problem
		...
		\problemAnswer{ % Answer
			...
			Since the last equation holds true, it $\implies$ $\{Y \leq y\} \in \mathcal{F}$ $\implies Y=F^-1$ is a RV. 
	...
	\begin{homeworkSection}{(4)} % Section within problem
		...
		\problemAnswer{ % Answer
			...
			Clearly this is nor necessarily equal to 1 so $fg$ is not a density function!
		}
	...
	\begin{homeworkSection}{(5)} % Section within problem
		...
		\problemAnswer{ % Answer
			...
			$\implies$ $\frac{-c}{-d+1} =1$ $\implies$ $c = d-1$ and $-d+1 \leq 1$ i.e $d \geq 0$ else the integral blows up to $\infty$
			
			Thus for $ d \geq 0 $ this is a density function satisfying $ c = d-1$
