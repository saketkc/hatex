...
\begin{homeworkProblem}[Exercise \# 3.3]
	...
	\begin{homeworkSection}{2}
	\problemAnswer{
	Given there are $c$ objects with $j$ chosen. The probability to select a new 'distinct' component given j are already selected is $\frac{c-j}{c}$ . Thus the distribution  is geometric with parameter $p=\frac{c-j}{c}$ and the mean being $\frac{1}{p}=\frac{c}{c-j}$
	}
	\end{homeworkSection}
