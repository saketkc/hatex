...
\begin{homeworkProblem}[Exercise \# 3.1] % Custom section title
	...
	\problemAnswer{
	 ...
	 $\sum_{1}^{\infty} \frac{C}{2^ii}=1$ Approximating summation by a definite integral: \\
	 $\int_{1}^{\infty} Cx{2^-x}=1 \implies C\int_{1}^{\infty}x2^-xdx =1$
	 Let $t =2^-x \implies dt = -2^-x ln 2 dt ; x = -\frac{ln t}{2} $
