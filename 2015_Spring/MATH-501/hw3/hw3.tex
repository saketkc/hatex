...
\begin{homeworkSection}{3}
	\problemAnswer{
		...
		$\left \|p-f\| \right_{\infty} = max|f(x)-p(x)| < \epsilon$ while $ a \leq x \leq b$
		...
		Thus,\\
		$|E_n(f)| = |\int_0^1 (f(x) - p(x)) dx + \sum_{i=1}^{n}w_i (p(x_i) - f(x_i))|$
		
		Applying triangular inequality,\\
		$E_n(f) \leq = \int_0^1 |(f(x) - p(x))| dx + \sum_{i=1}^{n}w_i |(p(x_i) - f(x_i))|
		\approx \left. \|p-f\| \right_{\infty} + \left. \|p-f\| \right_{\infty} 
		\leq \epsilon $\\
		Thus,\\ 
		
		 $\left. \|p-f\| \right_{\infty} \leq \frac{\epsilon}{2}$ and hence there exists a $N > 0$ such that $|E_n(f)| < \epsilon$  when $n > N$
