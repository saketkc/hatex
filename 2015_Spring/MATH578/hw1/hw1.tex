...
\begin{homeworkSection}{Question \# 3}
	\problemAnswer{
		...
		\begin{enumerate}
			\item All $|\beta|+1$ possible length $|\beta|$ substrings  possible  of length    $\beta\beta$ represents all possible circular rotations of $\beta$. This is intuitive, since a cicular rotation would involve concatenating the start of string to it's end
			...
			\item If P appears in $TT$, the $Z$ values the indices of it's start/end can be determined by querying all those
			points in the $Z$ value array, which exceed $|\alpha|$
		\end{enumerate}
