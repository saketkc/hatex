...
/* Query 5
*/
mysql> CREATE VIEW differential AS SELECT (round.score-tee.course_rating)*113/tee.slope_rating FROM round INNER JOIN tee ON round.tee = tee.name;

CREATE VIEW differential AS SELECT (round.score-tee.course_rating)*113/tee.slope_rating FROM round INNER JOIN tee WHERE round.tee = tee.name AND tee.course_name=round.course;

CREATE VIEW test AS SELECT * , (round.score-tee.course_rating)*113/tee.slope_rating FROM round INNER JOIN tee WHERE round.tee = tee.name AND tee.course_name=round.course;
